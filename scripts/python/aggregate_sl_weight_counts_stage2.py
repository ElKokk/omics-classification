#!/usr/bin/env python
"""
Aggregate SL weights for Stage 2: Count positive and highest per base per K.
"""
...
logging.info("✓ Positive counts (Stage 2) → %s", positive_out.name)

pd.concat(highest_frames).to_csv(highest_out, sep="\t", index=False)
logging.info("✓ Highest counts (Stage 2) → %s", highest_out.name)