...
# -*- coding: utf-8 -*-
"""
Stage‑2 · boxplot summary: distribution of metrics vs K, all models.
Creates three PNGs.

Input tables = per_split_metrics_k{K}.tsv (one per K; rows = test‑split × model)
"""
import matplotlib
matplotlib.use("Agg")
...
combined.rename(columns={"Sens": "Sensitivity", "Spec": "Specificity"},
                inplace=True)
...
def plot_metric(metric: str, out_png: Path) -> None:
    ...
    ax.set_title(textwrap.fill(
        f"{TITLE} | OUTER (distribution of {n_splits} frozen‑panel splits) | {metric}",
        80))
